            ...
            <div class="max-w-7xl mx-auto w-full relative z-10">
                ...
                <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-blue-200 text-sm mt-1 font-bold">Perbarui informasi dan detail publikasi</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data" 
                    x-data="{
                        form: {
                            ...
                            short_description: @js(old('short_description', $news->short_description)),
                            content: @js(old('content', $news->content)),
                            published_at: @js(old('published_at', $news->published_at ? $news->published_at->format('Y-m-d\TH:i') : '')),
                        },
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div class="bg-white hd-card border-none p-6 lg:p-10 space-y-8">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="title" x-model="form.title" @input="updateSlug" id="title_input" placeholder="Contoh: Gebyar Ramadhan 2025"
                                    class="w-full px-5 py-4 bg-[#EFF6FF] hd-wobbly-md border-hd-ink/10 text-[#1E3A8A] text-lg font-bold placeholder:text-gray-400 placeholder:font-normal focus:outline-none focus:border-[#1E3A8A] transition-all duration-200 @error('title') border-red-400 bg-red-50 @enderror" required>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="text-gray-400 text-sm hidden sm:inline font-bold">.../news/</span>
                                    <input type="text" name="slug" x-model="form.slug" id="slug_input" placeholder="gebyar-ramadhan-2025"
                                        class="flex-1 px-4 py-2 bg-gray-50 hd-wobbly-md border-hd-ink/10 text-gray-500 text-xs font-bold @error('slug') border-red-400 @enderror" required>
                                </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="short_description" x-model="form.short_description" id="short_description_input" rows="2" placeholder="Ringkasan singkat untuk tampilan list..."
                                    class="w-full px-5 py-4 bg-[#EFF6FF] hd-wobbly-md border-hd-ink/10 text-[#1E3A8A] text-sm font-bold placeholder:text-gray-400 placeholder:font-normal focus:outline-none focus:border-[#1E3A8A] transition-all duration-200 resize-none"></textarea>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="bg-white hd-wobbly-lg border-hd-ink/10 overflow-hidden focus-within:border-[#1E3A8A] transition-all">
                                    <textarea name="content" x-model="form.content" id="content_input" rows="15" placeholder="Tuliskan isi berita di sini..."
                                        class="w-full px-6 py-6 bg-transparent border-none outline-none text-[#1E3A8A] text-base font-bold leading-relaxed resize-none"></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6 lg:sticky lg:top-8">
                        ...
                        <div class="bg-white hd-card border-none p-6">
                            ...
                            <div class="relative group">
                                ...
                                <label for="image_input" class="flex flex-col items-center justify-center w-full h-48 lg:h-56 bg-gray-50 hd-wobbly-md border-hd-ink/10 cursor-pointer hover:bg-blue-50 transition-all overflow-hidden relative border-dashed border-2">
                                    <template x-if="!image_preview">
                                        <div class="flex flex-col items-center text-center px-4">
                                            ...
                                            <p class="text-gray-400 text-[10px] font-bold uppercase">Tap untuk Ubah</p>
                                        </div>
                                    </template>
                                    <template x-if="image_preview">
                        ...
                        {{-- Finalize Card --}}
                        <div class="bg-white hd-card border-none p-6 space-y-6">
                            {{-- Waktu Publish --}}
                            <div class="space-y-2">
                                ...
                                <input type="datetime-local" name="published_at" x-model="form.published_at" id="published_at_input"
                                    class="w-full px-4 py-3 bg-[#EFF6FF] hd-wobbly-md border-hd-ink/10 text-[#1E3A8A] text-xs font-bold focus:outline-none focus:border-[#1E3A8A] transition-all duration-200 @error('published_at') border-red-400 bg-red-50 @enderror" required>
                                @error('published_at') <p class="text-red-500 text-[10px] mt-1 font-bold">{{ $message }}</p> @enderror
                            </div>

                            {{-- Submit --}}
                            <div class="pt-4 space-y-3">
                                <x-atoms.button type="submit" variant="primary" size="lg" :handdrawn="true" class="w-full uppercase tracking-widest text-xs">
                                    Simpan Perubahan
                                </x-atoms.button>
                        ...
                        <x-atoms.button type="button" @click="showPreview = true" variant="primary" size="lg" :handdrawn="true" class="w-full group">
