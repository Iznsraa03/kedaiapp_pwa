<x-app-layout>
    ...
    <div class="min-h-screen bg-gray-50 flex flex-col">
        {{-- Header --}}
        <div class="px-5 lg:px-8 pt-12 lg:pt-16 pb-7 lg:pb-10 relative overflow-hidden bg-[#1E3A8A] hd-wobbly-lg mt-2 mx-2">
            ...
            <div class="max-w-7xl mx-auto w-full relative z-10">
                <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                        ...
                        <x-atoms.button href="{{ route('admin.activities.create') }}" variant="secondary" size="lg" :handdrawn="true" class="uppercase tracking-wide">
            ...
            <div class="max-w-7xl mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($activities as $activity)
                        <div class="group bg-white hd-card overflow-hidden flex flex-col h-full">
                            {{-- Stripe Status --}}
                            <div class="h-2 {{ $activity->stripeClass() }}"></div>

                            <div class="p-6 lg:p-8 flex flex-col h-full">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="w-14 h-14 bg-blue-50 hd-wobbly-md flex items-center justify-center text-3xl shadow-inner group-hover:scale-110 transition-transform border border-hd-ink/10">
                                        {{ $activity->emoji }}
                                    </div>
                                    <x-atoms.badge color="{{ str_contains($activity->badgeClass(), 'green') ? 'green' : (str_contains($activity->badgeClass(), 'red') ? 'red' : (str_contains($activity->badgeClass(), 'yellow') ? 'yellow' : 'blue')) }}" :handdrawn="true">
                                        {{ $activity->badgeLabel() }}
                                    </x-atoms.badge>
                                </div>
                                ...
                                <div class="space-y-2 mb-6">
                                    <div class="flex items-center gap-2 text-gray-500">
                                        ...
                                        <span class="text-xs font-semibold truncate">{{ $activity->location ?: 'Lokasi belum diset' }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 text-gray-500">
                                        <svg class="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    ...
                                    <div class="flex items-center gap-2 text-gray-500">
                                        ...
                                        <span class="text-xs font-bold text-[#2563EB] bg-blue-50 px-2 py-0.5 hd-wobbly-md border-hd-ink/10">{{ $activity->attendances_count }} Partisipan</span>
                                    </div>
                                </div>

                                <div class="mt-auto pt-6 hd-scribble-divider opacity-20 flex items-center gap-3">
                                    <x-atoms.button href="{{ route('admin.activities.show', $activity) }}" variant="primary" size="md" :handdrawn="true" class="flex-1">
                                        Scan QR
                                    </x-atoms.button>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.activities.edit', $activity) }}"
                                           title="Edit Kegiatan"
                                           class="w-12 h-12 flex items-center justify-center hd-wobbly-md border-2 border-gray-100 text-gray-400 hover:border-[#2563EB] hover:text-[#2563EB] hover:bg-blue-50 transition-all">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.activities.destroy', $activity) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kegiatan ini?')" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                                                title="Hapus"
                                                class="w-12 h-12 flex items-center justify-center hd-wobbly-md border-2 border-gray-100 text-gray-400 hover:border-red-500 hover:text-red-500 hover:bg-red-50 transition-all">
                                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @empty
                        <div class="col-span-full flex flex-col items-center justify-center gap-4 py-20 text-center bg-white hd-card border-none">
                            <div class="w-24 h-24 bg-[#EFF6FF] hd-wobbly-md border-[#2d2d2d]/10 flex items-center justify-center text-5xl mb-2">📭</div>
