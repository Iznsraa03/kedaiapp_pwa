<x-app-layout>
    ...
    {{-- ===== APPBAR ===== --}}
    <div class="px-5 pt-12 pb-8 relative overflow-hidden bg-[#1E3A8A] hd-wobbly-lg mt-2 mx-2">
        {{-- Dekorasi --}}
        ...
        <div class="relative z-10 flex items-center justify-start lg:justify-center">
            <div class="flex items-center gap-4">
                {{-- Logo Icon in Box --}}
                <div class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shrink-0 shadow-lg p-2.5">
                    <img src="/logo/KDCW.png" alt="KeDai Logo" class="w-full h-full object-contain">
                </div>
                {{-- Brand Name --}}
                <div class="flex flex-col">
                    <div class="flex items-baseline gap-2">
                       <span class="text-white text-3xl font-extrabold tracking-tight font-rockwell">KeDai</span>
    ...
    <div class="px-5 mt-4 mb-1">
        <div class="bg-white hd-card overflow-hidden">
            ...
            <div class="flex items-center gap-3 px-4 pt-4 pb-3">
                ...
                <a href="{{ route('profil') }}" class="flex-shrink-0">
                    @if(Auth::user()->avatar)
                        <img src="{{ Auth::user()->avatar_url }}" alt="{{ Auth::user()->name }}"
                            class="w-14 h-14 object-cover border-2 border-hd-ink hd-wobbly-md">
                    @else
                        <div
                            class="w-14 h-14 bg-[#EFF6FF] hd-wobbly-md flex items-center justify-center border-2 border-hd-ink">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <x-atoms.badge color="green" :handdrawn="true">Aktif</x-atoms.badge>
                </div>

            </div>

            {{-- Divider --}}
            <div class="mx-4 hd-scribble-divider"></div>
            ...
            <div class="flex items-center justify-between px-4 py-3 bg-[#F8FAFC]">
                ...
                <x-atoms.button variant="secondary" size="sm" :handdrawn="true" onclick="window.dispatchEvent(new CustomEvent('open-qr-sheet'))">
    ...
    <div class="px-5 py-4 space-y-4">
        ...
        <div>
            ...
            @if($activities->isEmpty())
                <div class="flex flex-col items-center gap-3 py-10 text-center bg-white hd-card m-2">
                    <div class="w-16 h-16 bg-bg-soft hd-wobbly-md border-[#2d2d2d]/10 flex items-center justify-center text-3xl">📭</div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                    @foreach($activities as $activity)
                        <a href="{{ route('activities.show', $activity) }}"
                            class="block bg-white hd-card overflow-hidden active:scale-[0.98] lg:hover:shadow-md transition-all duration-200">
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex items-center gap-3 mb-3">
                                    <div class="flex items-center gap-1.5 bg-[#EFF6FF] px-2.5 py-1.5 hd-wobbly-md border-[#2d2d2d]/10 text-xs text-[#1E3A8A]">
                                        ...
                                        <span class="font-bold">{{ $activity->starts_at->translatedFormat('d M Y') }}</span>
                                    </div>
                                ...
                                {{-- Divider --}}
                                <div class="hd-scribble-divider opacity-20 mb-3"></div>
                                ...
                                <div class="grid grid-cols-[1fr_auto] items-center gap-3">
                                    ...
                                    <x-atoms.button variant="primary" size="sm" :handdrawn="true">
                                        Detail →
                                    </x-atoms.button>
                                </div>
                            </div>
