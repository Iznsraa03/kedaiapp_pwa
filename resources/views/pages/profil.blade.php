<x-app-layout>
    ...
    {{-- Header --}}
    <div class="px-5 pt-12 pb-10 bg-[#1E3A8A] hd-wobbly-lg mt-2 mx-2">
        <div class="flex flex-col items-center gap-3">
            @if(Auth::user()->avatar)
            <div class="w-20 h-20 hd-wobbly-md overflow-hidden border-2 border-white/30 shadow-lg">
                <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="Foto Profil" class="w-full h-full object-cover">
            </div>
            @else
            <div class="w-20 h-20 bg-white/20 hd-wobbly-md flex items-center justify-center">
    ...
    <a href="{{ route('admin.activities.index') }}"
       class="flex items-center gap-3 bg-[#EFF6FF] hd-card p-4 active:scale-[0.98] transition-transform duration-150 border-none">
        <div class="w-11 h-11 bg-[#2563EB] hd-wobbly-md flex items-center justify-center flex-shrink-0">
    ...
    {{-- Menu Profil --}}
        <a href="{{ route('profil.edit') }}"
           class="bg-white hd-card p-4 flex items-center gap-4 active:scale-[0.98] transition-transform duration-150 border-none">
            <div class="w-11 h-11 bg-[#EFF6FF] hd-wobbly-md flex items-center justify-center flex-shrink-0">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <x-atoms.button type="submit" variant="danger" size="lg" :handdrawn="true" class="w-full mt-2">
