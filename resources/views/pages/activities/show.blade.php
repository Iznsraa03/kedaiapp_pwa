<x-app-layout>
    ...
    {{-- Header --}}
    <div class="bg-[#1E3A8A] px-5 pt-12 pb-16 relative overflow-hidden hd-wobbly-lg mt-2 mx-2">
        <div class="absolute -top-6 -right-6 w-36 h-36 rounded-full border-[24px] border-white/10 pointer-events-none"></div>
        
        <div class="relative z-10 flex items-center justify-between mb-4">
            <a href="{{ route('home') }}" class="w-10 h-10 bg-white/20 hd-wobbly-md flex items-center justify-center text-white active:scale-90 transition-all border border-transparent">
    ...
    <div class="px-5 -mt-8 space-y-5 pb-12">
        ...
        {{-- Info Grid --}}
        <div class="bg-white hd-card p-5 space-y-4 border-none">
            <div class="flex items-center gap-4">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-gray-400 text-xs mt-0.5 font-bold uppercase tracking-wide">
                        {{ $activity->starts_at->format('H:i') }} – {{ $activity->ends_at->format('H:i') }} WIB
                    </p>
                </div>
            </div>
 
            <div class="hd-scribble-divider opacity-10 mx-2"></div>
 
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-red-50 hd-wobbly-md flex items-center justify-center flex-shrink-0 border border-hd-ink/10">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-gray-400 text-xs mt-0.5 font-bold uppercase tracking-wide">Lokasi Kegiatan</p>
                </div>
            </div>
        </div>
div>
    ...
    <x-atoms.button @click="loading = true; setTimeout(() => { showQr = true; loading = false }, 800)" variant="primary" size="lg" :handdrawn="true" class="w-full group">
        ...
        <template x-if="loading">
            <svg class="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
                ...
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    ...
                    </d                    {{-- QR Display (Modern Digital Pass Style) --}}
                    <div x-show="showQr" 
                        x-transition:enter="transition cubic-bezier(0.34, 1.56, 0.64, 1) duration-500" 
                        x-transition:enter-start="opacity-0 scale-95 translate-y-10"
                        x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                        class="bg-white hd-card overflow-hidden relative border-none"
                    >
                        {{-- Pass Header --}}
                        <div class="bg-[#1E3A8A] px-6 py-5 text-center relative overflow-hidden hd-wobbly-lg -mt-2 -mx-2">
                            <div class="absolute top-0 right-0 w-24 h-24 bg-white/5 rounded-full -mr-10 -mt-10"></div>
                            <div class="relative z-10">
                                <h4 class="text-white font-black text-sm uppercase tracking-[0.2em] font-patrick">Presensi Digital</h4>
                        ...
                        <div class="p-8 flex flex-col items-center">
                            {{-- QR Frame --}}
                            <div class="relative p-2 bg-white hd-wobbly-lg shadow-inner border border-hd-ink/10 mb-6 group">
                                <div class="bg-white p-5 rounded-[2rem] shadow-xl relative z-10 border border-hd-ink/5">
                                    {!! QrCode::size(170)->margin(0)->color(30, 58, 138)->generate(auth()->user()->generateAttendanceToken($activity)) !!}
                                </div>
                                {{-- Corner Decorations --}}
                                <div class="absolute -top-1 -left-1 w-6 h-6 border-t-4 border-l-4 border-hd-ink rounded-tl-xl opacity-20"></div>
                                <div class="absolute -bottom-1 -right-1 w-6 h-6 border-b-4 border-r-4 border-hd-ink rounded-br-xl opacity-20"></div>
                            </div>
 
                            {{-- Status & Timer --}}
                            <div class="flex flex-col items-center gap-3 w-full">
                                <x-atoms.badge color="green" :handdrawn="true">
                                    Valid 5 Menit
                                </x-atoms.badge>
                                <p class="text-gray-400 text-[10px] font-bold max-w-[220px] leading-relaxed text-center">
                        ...
                        <div class="flex items-center px-4">
                            <div class="w-6 h-6 bg-gray-50 rounded-full -ml-8 border border-hd-ink/10"></div>
                            <div class="flex-1 hd-scribble-divider opacity-10"></div>
                            <div class="w-6 h-6 bg-gray-50 rounded-full -mr-8 border border-hd-ink/10"></div>
                        </div>
 
                        {{-- Pass Footer --}}
                        <div class="p-4 bg-gray-50/50">
                            <button @click="showQr = false" class="w-full py-3 text-gray-400 text-[10px] font-bold uppercase tracking-[0.2em] hover:text-[#1E3A8A] transition-colors">
                                Sembunyikan QR
                            </button>
                        </div>
                    </div>
